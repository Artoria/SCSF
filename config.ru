...
Dir.chdir("D:\\Backup\\Documents\\RPGXP")
module Helper
  def makeres str_or_arr
    return str_or_arr if Array === str_or_arr
    [200, {}, [str_or_arr]]
  end
    
  def httpok(response = "")
    response = makeres response
    response[0] = 200
    response
  end
  ...
  def load_script(filename)
    v = load_data filename
    v.each{|x| x[3] = Zlib::Inflate.inflate(x[2]).force_encoding("UTF-8")}
  end
  def recursive_method(obj, path = "", &b) 
    obj.methods.grep(/[A-Za-z]+=/).each{|sym|
      if obj.respond_to?(sym.to_s.chomp("="))
        attr = obj.send(sym.to_s.chomp("="))
        case attr
          when Numeric, String
            yield attr, path + ".#{sym}", sym 
          else
            recursive_method attr, path + ".#{sym}", &b 
        end
...
Rack::Builder.send :include, Helper

run Rack::Builder.new {
  use Rack::ShowExceptions
  map "/list" do
    files = JSON.dump Dir.glob("*").select(&FileTest.method(:directory?))
    run lambda{|env| httpok json files }
  end

  map "/goto" do
    run lambda{|env| 
         x = CGI.parse(env["QUERY_STRING"])["path"].first
         Dir.chdir x
         httpok json JSON.dump x 
     }
  end

  map "/current" do
    run lambda{|env| httpok json JSON.dump Dir.getwd }
  end

  map "/find" do
    run lambda{|env| 
         str = CGI.parse(env["QUERY_STRING"])["wd"].first
         result = {}
         t = Time.now
         Dir.glob("**/Scripts.{rxdata, rvdata, rvdata2}").each{|filename|
             load_script(filename).each{|(id, name, _, text)|
                text.split(/\n/).each_with_index{|v, line|
                   if v[str]
                     result[ [filename, name, line] ] = v
                   end
                }
             }
         }

        Dir.glob("**/*.{rxdata, rvdata, rvdata2}").each{|filename|
             next unless Array === (a = load_data(filename))
             a.each_with_index{|object, index|
                next unless object
                recursive_method object, "[%d]" % index do |value, path, sym|
                   next if !value.to_s[str]
                   result[ [filename, File.basename(filename, ".*"), path].map{|x| x.force_encoding("UTF-8")} ] = value.to_s.force_encoding("UTF-8")
                end 
             }
         }
         
         httpok json JSON.dump [[["Finished in %0.6fs" % (Time.now - t), "", ""], ""]]  + result.to_a 
     }
  end

  map "/app" do
     run lambda{|env|
        httpok html File.read File.join(File.dirname(__FILE__), "app.html")
     }
  end
}
